...
const createConsultationSchema = Joi.object({
    ...
    symptoms: Joi.array()
        .items(Joi.string().max(100).messages({
            'string.base': 'Cada síntoma debe ser un texto.',
            'string.max': 'Cada síntoma no puede superar los 100 caracteres.'
        }))
        .default([])
        .messages({
            'array.base': 'El campo síntomas debe ser una lista de textos.'
        }),

    diagnosisSummary: Joi.string()
        .max(500)
        .allow('', null)
        .messages({
            'string.max': 'El resumen del diagnóstico no puede superar los 500 caracteres.'
        }),
